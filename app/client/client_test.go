...
var errTooManyRedirects = errors.New("执行请求时检测到太多重定向")
...
func TestClientGetWithBody(t *testing.T) {
	...
	err := c.Do(context.Background(), req, res)
	assert.Nil(t, err)
	assert.Equal(t, "test", string(res.Body()))
}
...
func TestClientPostBodyStream(t *testing.T) {
	...
	cStream, _ := NewClient(
		WithDialer(newMockDialerWithCustomFunc(opt.Network, opt.Addr, 1*time.Second, nil)),
	...
	assert.Equal(t, "a="+v, string(body))
}
...
func TestClientNilResp(t *testing.T) {
	...
	err = c.DoTimeout(context.Background(), req, nil, time.Second)
	assert.Nil(t, err)
}
...
func TestClientParseConn(t *testing.T) {
	...
	localAddr := res.LocalAddr().String()
	if !regexp.MustCompile(`^127\.0\.0\.1:[0-9]{4,5}$`).MatchString(localAddr) {
...
func TestClientPostArgs(t *testing.T) {
	...
	args := req.PostArgs()
	args.Add("username", "mike")
	args.Add("age", "18")
	req.Header.SetMethod(consts.MethodPost)
	req.SetRequestURI("http://make.wind.great?again")
	err := c.Do(context.Background(), req, res)
	assert.Nil(t, err)
	if len(res.Body()) == 0 {
		t.Fatal("无法设置正文参数")
...
func TestClientHeaderCase(t *testing.T) {
	...
	c, _ := NewClient(
		WithDialer(newMockDialerWithCustomFunc(opt.Network, opt.Addr, time.Second, nil)),
...
func TestClientReadTimeout(t *testing.T) {
	...
	engine.GET("/", func(c context.Context, ctx *app.RequestContext) {
		if timeout {
			fmt.Println("第二次进入，即将休眠 1 分钟")
			time.Sleep(time.Minute)
			fmt.Println("休眠完成")
		} else {
			fmt.Println("第二次进入，将超时设为 true")
			timeout = true
		}
	})
	go engine.Run()
	...
	case <-time.After(c.ReadTimeout*time.Duration(c.RetryConfig.MaxAttemptTimes) + time.Second*2):
		t.Fatal("Client.ReadTimeout 不起作用")
	}
}
...
func TestHostClientMaxConnDuration(t *testing.T) {
	...
	val := atomic.LoadUint32(&connectionCloseCount)
	if val == 0 {
...
func TestSetFiles(t *testing.T) {
	...
	err := client.DoTimeout(context.Background(), req, rsp, 100*time.Second)
	if err != nil {
		t.Error(err)
	}
}
...
func TestClientProxyWithStandardDialer(t *testing.T) {
	...
	for _, testCase := range testCases {
		...
		t.Run(fmt.Sprintf("httpsSite=%v, httpsProxy=%v", httpsSite, httpsProxy), func(t *testing.T) {
			...
			// 如果两台服务器都不是 HTTPS,或者两台服务器都是 HTTPS,那么 c 可以来自任意一台。
			// 如果只有一台服务器是 HTTPS,则 c 必须来自那台 HTTPS 服务器, 以确保它配置为使用 testcert.go 中的假根 CA。
...
func TestClientLastMiddleware(t *testing.T) {
	...
	mw0 := func(next Endpoint) Endpoint {
		return func(ctx context.Context, req *protocol.Request, resp *protocol.Response) (err error) {
			...
			assert.Equal(t, "final3", finalValue0)
			finalValue1 := ctx.Value("final1")
			assert.Equal(t, "final1", finalValue1)
			finalValue2 := ctx.Value("final2")
			assert.Equal(t, "final2", finalValue2)
			return nil
		}
	}
	mw1 := func(next Endpoint) Endpoint {
	...
	assert.Equal(t, errorLastMiddlewareExist, err)
	client.Use(mw1)
	client.Use(mw2)
	client.Use(mw3)
	client.Use(mw4)
	...
	assert.Equal(t, reflect.ValueOf(last).Pointer(), reflect.ValueOf(mw0).Pointer())
	last = client.TakeOutLastMiddleware()
	assert.Nil(t, last)
}
...
func TestClientReadResponseBodyStreamWithConnectionClose(t *testing.T) {
	...
	assert.Equal(t, part1, string(resp.Body()))
	...
	assert.Equal(t, part1, string(resp1.Body()))
}
...
func TestClientState(t *testing.T) {
	...
	client, _ := NewClient(
		WithConnStateObserve(func(hcs config.HostClientState) {
			...
			case int32(0):
				assert.Equal(t, 1, hcs.ConnPoolState().TotalConnNum)
				assert.Equal(t, 1, hcs.ConnPoolState().PoolConnNum)
				assert.Equal(t, "127.0.0.1:11000", hcs.ConnPoolState().Addr)
				atomic.StoreInt32(&state, int32(1))
			case int32(1):
				assert.Equal(t, 0, hcs.ConnPoolState().TotalConnNum)
				assert.Equal(t, 0, hcs.ConnPoolState().PoolConnNum)
				assert.Equal(t, "127.0.0.1:11000", hcs.ConnPoolState().Addr)
