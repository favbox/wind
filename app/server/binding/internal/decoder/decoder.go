...
func getFieldDecoder(field reflect.StructField, index int, parentIdx []int, parentJSONName string, byTag string, config *DecodeConfig) ([]fieldDecoder, bool, error) {
	...
	if field.Type.Kind() == reflect.Map {
		dec, err := getMapFieldDecoder(field, index, fieldTagInfos, parentIdx, config)
		return dec, needValidate, err
	}
	...
	if field.Type.Kind() == reflect.Struct {
		...
		for i := 0; i < el.NumField(); i++ {
			...
			var indices []int
			if len(parentIdx) > 0 {
				indices = append(indices, parentIdx...)
			}
			indices = append(indices, index)
			dec, needValidate2, err := getFieldDecoder(el.Field(i), i, indices, newParentJSONName, byTag, config)
			needValidate = needValidate || needValidate2
			if err != nil {
				return nil, false, err
			}
