...
type mapTypeFieldDecoder struct {
	fieldInfo
}

func (d *mapTypeFieldDecoder) Decode(req *protocol.Request, params param.Params, refValue reflect.Value) error {
	var err error
	var text string
	var exists bool
...
func getMapFieldDecoder(field reflect.StructField, index int, tagInfos []TagInfo, parentIdx []int, config *DecodeConfig) ([]fieldDecoder, error) {
	...
	return []fieldDecoder{&mapTypeFieldDecoder{
		fieldInfo: fieldInfo{
			index:       index,
