...
import (
	...
	"github.com/favbox/wind/common/testdata/proto"
...
func TestSetValidator(t *testing.T) {
	...
	c := NewContext(0)
	c.SetValidator(m)
	c.SetBinder(binding.NewBinder(&binding.BindConfig{Validator: m}))
	type User struct {
		Age int `vt:"$>=0&&$<=130"`
	}
	...
	err := c.Validate(user)
	assert.NotNil(t, err)
	assert.Equal(t, "test mock", err.Error())
}
...
func TestRequestContext_PureJSON(t *testing.T) {
	...
	assert.Equal(t, "{\"html\":\"<b>Hello World</b>\"}\n", string(ctx.Response.Body()))
}
...
func TestNewContext(t *testing.T) {
	...
	ctx := reqContext
	assert.Equal(t, "testValue", ctx.Value("testContextKey"))
}
...
func TestWrite(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, 9, l)
	assert.Equal(t, "test body", string(ctx.Response.BodyBytes()))
}
...
func TestRequestContext_NotFound(t *testing.T) {
	...
	assert.Equal(t, "404 Page not found", string(ctx.Response.BodyBytes()))
}

func TestRequestContext_Redirect(t *testing.T) {
	ctx := NewContext(0)
	ctx.Redirect(consts.StatusFound, []byte("/hello"))
	assert.Equal(t, consts.StatusFound, ctx.Response.StatusCode())
...
func TestGetRedirectStatusCode(t *testing.T) {
	...
	assert.Equal(t, consts.StatusFound, val)
}
...
func TestRequestContext_IsPost(t *testing.T) {
	ctx := NewContext(0)
	ctx.Request.Header.SetMethod(consts.MethodPost)
	assert.True(t, ctx.IsPost())
	assert.Equal(t, consts.MethodPost, string(ctx.Method()))
}

func TestRequestContext_IsGet(t *testing.T) {
	ctx := NewContext(0)
	ctx.Request.Header.SetMethod(consts.MethodPost)
	assert.False(t, ctx.IsGet())
	ctx.Request.Header.SetMethod(consts.MethodGet)
	assert.Equal(t, consts.MethodGet, string(ctx.Method()))
}
...
func TestContextRenderFileFromFS(t *testing.T) {
	...
	assert.Equal(t, consts.MIMETextPlainUTF8, string(ctx.Response.Header.Peek("Content-Type")))
...
func TestContextRenderAttachment(t *testing.T) {
	...
	ctx := NewContext(0)
	var r protocol.Request
	r.Header.SetMethod(consts.MethodGet)
	r.SetRequestURI("/")
	r.CopyTo(&ctx.Request)
	...
	assert.Equal(t, fmt.Sprintf("attachment; filename=\"%s\"", newFilename),
...
func TestContextError(t *testing.T) {
	...
	assert.Equal(t, errs.ErrorTypePrivate, c.Errors[0].Type)

	assert.Equal(t, secondErr, c.Errors[1].Err)
	assert.Equal(t, "some data 2", c.Errors[1].Meta)
	assert.Equal(t, errs.ErrorTypePublic, c.Errors[1].Type)
...
func TestContextAbortWithStatus(t *testing.T) {
	...
	assert.Equal(t, rConsts.AbortIndex, c.index)
	assert.Equal(t, consts.StatusUnauthorized, c.Response.Header.StatusCode())
	assert.True(t, c.IsAborted())
}
...
func TestContextAbortWithStatusJSON(t *testing.T) {
	...
	assert.Equal(t, rConsts.AbortIndex, c.index)
	assert.Equal(t, consts.StatusUnsupportedMediaType, c.Response.Header.StatusCode())
	assert.True(t, c.IsAborted())

	contentType := c.Response.Header.Peek("Content-Type")
	assert.Equal(t, consts.MIMEApplicationJSONUTF8, string(contentType))

	jsonStringBody := c.Response.Body()
	assert.Equal(t, "{\"foo\":\"fooValue\",\"bar\":\"barValue\"}", string(jsonStringBody))
}
...
func TestContextSetGetValues(t *testing.T) {
	...
	assert.Equal(t, c.MustGet("uint32").(uint32), uint32(42))
	assert.Equal(t, c.MustGet("uint64").(uint64), uint64(42424242424242))
...
func TestContextGetInt(t *testing.T) {
	...
	c.Set("string", "this is a string")
	assert.Equal(t, 0, c.GetInt("string"))
}

func TestContextGetInt32(t *testing.T) {
	c := &RequestContext{}
	c.Set("int32", int32(-42))
	assert.Equal(t, int32(-42), c.GetInt32("int32"))
	c.Set("string", "this is a string")
	assert.Equal(t, int32(0), c.GetInt32("string"))
}
...
func TestContextGetInt64(t *testing.T) {
	...
	c.Set("string", "this is a string")
	assert.Equal(t, int64(0), c.GetInt64("string"))
}
...
func TestContextGetUint(t *testing.T) {
	...
	c.Set("string", "this is a string")
	assert.Equal(t, uint(0), c.GetUint("string"))
}

func TestContextGetUint32(t *testing.T) {
	c := &RequestContext{}
	c.Set("uint32", uint32(42))
	assert.Equal(t, uint32(42), c.GetUint32("uint32"))
	c.Set("string", "this is a string")
	assert.Equal(t, uint32(0), c.GetUint32("string"))
}

func TestContextGetUint64(t *testing.T) {
	c := &RequestContext{}
	c.Set("uint64", uint64(42424242424242))
	assert.Equal(t, uint64(42424242424242), c.GetUint64("uint64"))
	c.Set("string", "this is a string")
	assert.Equal(t, uint64(0), c.GetUint64("string"))
}

func TestContextGetFloat32(t *testing.T) {
	c := &RequestContext{}
	c.Set("float32", float32(4.2))
	assert.Equal(t, float32(4.2), c.GetFloat32("float32"))
	c.Set("string", "this is a string")
	assert.Equal(t, float32(0.0), c.GetFloat32("string"))
}
...
func TestContextGetFloat64(t *testing.T) {
	...
	c.Set("string", "this is a string")
	assert.Equal(t, 0.0, c.GetFloat64("string"))
}
...
func TestContextGetTime(t *testing.T) {
	...
	c.Set("string", "this is a string")
	assert.Equal(t, time.Time{}, c.GetTime("string"))
}
...
func TestContextGetDuration(t *testing.T) {
	...
	c.Set("string", "this is a string")
	assert.Equal(t, time.Duration(0), c.GetDuration("string"))
}
...
func TestContextGetStringSlice(t *testing.T) {
	...
	c.Set("string", "this is a string")
	var expected []string
	assert.Equal(t, expected, c.GetStringSlice("string"))
}
...
func TestContextGetStringMap(t *testing.T) {
	...
	assert.Equal(t, expected, c.GetStringMap("string"))
}
...
func TestContextGetStringMapString(t *testing.T) {
	...
	assert.Equal(t, expected, c.GetStringMapString("string"))
}
...
func TestContextGetStringMapStringSlice(t *testing.T) {
	...
	c.Set("string", "this is a string")
	var expected map[string][]string
	assert.Equal(t, expected, c.GetStringMapStringSlice("string"))
}
...
func TestContextTraceInfo(t *testing.T) {
	...
	assert.Equal(t, traceIn, traceOut)
}
...
func TestGetHeader(t *testing.T) {
	...
	assert.Equal(t, consts.MIMETextPlainUTF8, string(val))
}
...
func TestBindForm(t *testing.T) {
	...
	c := &RequestContext{}
	c.Request.SetRequestURI("/foo/bar?a=111&b=222")
...
func TestSetBinder(t *testing.T) {
	...
	err = c.BindAndValidate(&req)
	assert.NotNil(t, err)
	assert.Equal(t, "test binder", err.Error())
...
func TestRequestContext_SetCookie(t *testing.T) {
	c := NewContext(0)
	c.SetCookie("user", "wind", 1, "/", "localhost", protocol.CookieSameSiteLaxMode, true, true)
	assert.Equal(t, "user=wind; max-age=1; domain=localhost; path=/; HttpOnly; secure; SameSite=Lax", c.Response.Header.Get("Set-Cookie"))
}

func TestRequestContext_SetCookiePathEmpty(t *testing.T) {
	c := NewContext(0)
	c.SetCookie("user", "wind", 1, "", "localhost", protocol.CookieSameSiteDisabled, true, true)
	assert.Equal(t, "user=wind; max-age=1; domain=localhost; path=/; HttpOnly; secure", c.Response.Header.Get("Set-Cookie"))
}
...
func TestRequestContext_VisitAll(t *testing.T) {
	...
	t.Run("VisitAllCookie", func(t *testing.T) {
		...
		assert.Equal(t, []string{"aaa", "bbb", "ccc", "ddd"}, s)
	})
