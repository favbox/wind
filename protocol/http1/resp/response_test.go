...
func TestResponseBodyStreamErrorOnPanicDuringClose(t *testing.T) {
	...
	err := Write(&resp, zw)
	if err == nil {
		t.Fatalf("期望错误发生于写响应时")
	}
	e, ok := err.(*ErrBodyStreamWritePanic)
	if !ok {
		t.Fatalf("期望的错误结构体为 *ErrBodyStreamWritePanic, 得到: %+v.", e)
	}
	if e.Error() != "恐慌发生于写入正文流的时候：正在关闭错误的正文流" {
		t.Fatalf("意外错误值, 得到: %+v.", e.Error())
	}
}
...
func TestResponseBodyStreamErrorOnPanicDuringRead(t *testing.T) {
	...
	if e.Error() != "恐慌发生于写入正文流的时候：正在读取错误的正文流" {
