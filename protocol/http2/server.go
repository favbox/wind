...
const (
	...
	firstSettingsTimeout   = 2 * time.Second // 无论如何，这些帧应该在握手过程中进行传输。
...
// Server 表示 HTTP/2 服务器。实现 protocol.Server 协议接口。
type Server struct {
	BaseEngine

	// 构建一个连接的写入调度器。 若为 nil，则选择默认的调度器。
	NewWriteScheduler func() WriterScheduler

	// 内部状态。这是一个指针（而不是直接嵌入），
	// 以便我们不在这个结构体中嵌入一个 Mutex，这将使结构体不可复制，可能会破坏一些调用者的代码。
...
// // maxQueuedControlFrames 是在关闭连接之前将排队等待写入的控制帧
// （比如 SETTINGS、PING和RST_STREAM）的最大数量，以防止内存耗尽攻击。
...
// Serve 提供 HTTP/2 连接服务，并阻止该连接直至不再可读。
//
// 当 onData 开始时，若 c 无任何读写操作则会写入初始的设置帧，
// 并期望能从客户端读取和设置前导帧。若 c 有类似 *tls.Conn 的 ConnectionState 方法，
// 则用其验证 TLS 密码套件，并在 Handlers 中设置 Request.TLS 字段。
...
func (e *BaseEngine) AcquireReqCtx() *app.RequestContext {
	return e.Core.GetCtxPool().Get().(*app.RequestContext)
}

func (e *BaseEngine) ReleaseReqCtx(ctx *app.RequestContext) {
	e.Core.GetCtxPool().Put(ctx)
}
