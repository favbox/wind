...
func makeACall(t *testing.T, method, url string, header http.Header, body string, expectStatusCode int, expectCookieValue []byte) {
	...
	assert.Equal(t, expectCookieValue, cookie.Value())
}
...
func TestCopyToWinRequest(t *testing.T) {
	req := http.Request{
		Method:     "GET",
		RequestURI: "/test",
		URL: &url.URL{
	...
	assert.Equal(t, req.Proto, windReq.Header.GetProtocol())
	assert.Equal(t, req.Header.Get("key1"), windReq.Header.Get("key1"))
	valueSlice := make([]string, 0, 2)
	windReq.Header.VisitAllCustomHeader(func(key, value []byte) {
		if strings.ToLower(string(key)) == "key2" {
			valueSlice = append(valueSlice, string(value))
		}
	})

	assert.Equal(t, req.Header.Values("key2"), valueSlice)

	assert.Equal(t, 3, windReq.Header.Len())
}
