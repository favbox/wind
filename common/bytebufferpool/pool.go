...
// Get 从池中取一个空字节缓冲区。用后 Put 回池，以减少内存分配次数。
func Get() *ByteBuffer { return defaultPool.Get() }

// Put 将字节缓冲区放回池中。放后勿碰，以防竞争。
func Put(b *ByteBuffer) { defaultPool.Put(b) }

// Pool 表示字节缓冲池。不同类型用不同池，正确确定类型可减少内存浪费。
type Pool struct {
	calls       [steps]uint64
...
// Get 返回一个长度为零的新字节缓冲区。 用后 Put 回池，可减少 GC 开销。
...
// Put 将通过 Get 获取的字节缓冲区归还池中，还后勿碰，以防竞争。
